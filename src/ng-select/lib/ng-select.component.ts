...
export class NgSelectComponent implements OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor {
    ...
    @Input()
    get clearSearchOnAdd() { return isDefined(this._clearSearchOnAdd) ? this._clearSearchOnAdd : (!this.multiple && this.closeOnSelect); };
    ...
    select(item: NgOption) {
        ...
        if (this.closeOnSelect || (this.itemsList && this.itemsList.noItemsToSelect)) {
            this.close();
        }
    }
    ...
    private _selectInput() {
        ...
        for (const item of this.itemsList.filteredItems) {
            if (item && item.label && item.label.toLowerCase() === term) {
